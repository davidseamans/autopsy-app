create or replace function public.current_user_can_enter_paid_autopsy()
returns boolean
language sql
stable
security definer
...
as $$
  select auth.uid() is not null and exists (
    ...
    left join public.autopsy_runs r on r.id = e.autopsy_run_id
...
revoke all on function public.current_user_can_enter_paid_autopsy()
  from public, anon;
grant execute on function public.current_user_can_enter_paid_autopsy()
...
begin
  ...
  if v_is_preview then
    ...
    -- Resume before consuming. A second paid entitlement belongs to a future
    -- assessment and must not be consumed by a reload of the current run.

...
describe("Stage 1 aggregate lead to quote funnel", () => {
  ...
  it("captures customer details at quote creation and preserves quote-to-job routing", () => {
    ...
    expect(quotesPage).toContain("Potential — unquoted");
    expect(quotesPage).toContain("Capture only enough detail to arrange the appointment");
    expect(quotesPage).toContain('label="Potential"');
    expect(quotesPage).toContain('label="All generated"');
    expect(quotesPage).toContain("Showing ${filteredQuotes.length} of ${counts.sent}");
    expect(quotesPage).toContain("type GeneratedQuoteStatus");
    ...
    expect(quotesPage).toContain('source: "Contact"');
    expect(quotesPage).not.toContain("saveStage1LeadCount");
    expect(quotesPage).not.toContain("createStage1LeadActivity");

...
export function buildAccountantPackFiles({ units, business, runId, generatedAt = new Date() }: PackInput) {
...
export function zipAccountantPack(files: Record<string, string>) {
...
export function downloadAccountantPack(input: PackInput) {
  const blob = zipAccountantPack(buildAccountantPackFiles(input)); const url = URL.createObjectURL(blob); const anchor = document.createElement("a");

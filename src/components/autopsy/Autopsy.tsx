...
const CANDIDATE_DIMENSION_FINDINGS: Record<string, { positive: string; concern: string; consequence: string }> = {
  ...
  market_reality: {
    positive: "You described real customer action—not only encouragement—that suggests people may pay for the service.",
    concern: "Your answers did not yet describe a firm customer decision to choose and pay you.",
    consequence: "Buying equipment or leaving steady work before customers act can turn an idea into an expensive guess.",
  },
  ...
  execution_discipline: {
    positive: "You described completed practical actions and consistent follow-through.",
...
function evidenceLabel(score: number): string {
  if (score >= 5) return "Strong answer";
  if (score >= 3) return "Developing answer";
  if (score >= 2) return "Early answer";
  return "Not clear yet";
}

function candidateSnapshotStatus(score: number): string {
  if (score >= 5) return "Strong enough to test";
  if (score >= 3) return "Some parts remain unclear";
...
function CandidateVerdict({
  ...
  const explanationProfile = ready
    ...
    : band === "not_viable"
      ? {
          decision: "This is not the same result as Stop. Your answers showed some useful signs, but important parts were still too thin or uneven to rely on when customers, money and pressure become real. Autopsy cannot responsibly recommend a test run yet.",
          distinction: "Not Ready does not rule out your potential. It means the present picture is not yet steady enough. Enthusiasm, intention and a plausible idea cannot substitute for dependable follow-through, financial room and practical experience.",
          ...
          questions: [
            ...
            "What useful experience or changed circumstance could make the picture clearer?",
          ],
        }
      : band === "critical_stop"
        ? {
            ...
            distinction: "Stop is a successful Autopsy outcome. Its value is the mistake it may prevent. It does not say you can never become a capable owner; it says the responsible decision, based on what you told Autopsy today, is not to proceed.",
        ...
        : {
            decision: "Your answers contain some encouraging signs, but important assumptions remain unresolved. Autopsy is not yet recommending an unrestricted move into business ownership.",
            distinction: "This result sits between an outright stop and readiness for a controlled test. It calls for caution, clarification and more practical learning before serious commitments are made.",
            risk: "The danger is treating promising signs as certainty. Any next step should remain small, reversible and protective of your present income and obligations.",
            questions: [
              "Which assumption most needs practical experience?",
              "What is the smallest safe way to learn more about it?",
              "What commitment should wait until the position is clearer?",
            ],
          };
    ...
    : band === "critical_stop"
      ...
      : band === "not_viable"
        ? scoreValue != null && scoreValue >= 9
          ? "Some early foundations are visible, but they are not yet dependable enough to carry customers, costs and pressure. Your task is to turn intention into sustained practical experience."
          : "Most of the essential foundations are not yet clear. Keep your present security and build practical exposure before treating ownership as an immediate plan."
        : band === "high_risk"
          ? scoreValue != null && scoreValue >= 18
            ? "You have a developing base, but the remaining gaps could still undo the stronger areas under pressure. Concentrated practical learning is more useful than broad preparation now."
            : "There is genuine potential, but it remains fragile. Several assumptions still need to become observable habits or practical experience before a test run is responsible."
          : scoreValue != null && scoreValue >= 27
            ? "You are approaching test readiness. The remaining task is not more enthusiasm or information; it is greater practical clarity in the few areas that still weaken the whole picture."
            : "You are closer, but readiness is still uneven. A small number of areas need practical strengthening before First 5 Jobs becomes sensible.";
  ...
  const printExplanation = () => {
    ...
    const disclosureNote = showAuditAppendix
      ...
      : "This fuller explanation supports the decision shown on screen. It is based on what you told Autopsy today; it is not a demand for documents or an external inspection. It does not disclose Autopsy scoring rules or provide an answer key. A future result should change only when the underlying experience, circumstances or actions have genuinely changed.";
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-5 pb-10">
      <section className={cn("rounded-2xl border-2 p-6 text-center shadow-sm sm:p-8", colour)}>
        ...
        <p className="mt-3 text-sm opacity-75">A decision based on what you told Autopsy today</p>
      </section>

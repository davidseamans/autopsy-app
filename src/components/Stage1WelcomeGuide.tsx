...
const quotesSlides = [
  ...
  { title: "Print or email the quote", narration: "Open a quote to review the actual document. Use Print or save PDF to print it, or save the PDF and attach it to an email. The customer sees a proper quote using your verified business details." },
  { title: "Convert only when accepted", narration: "When the customer genuinely accepts, change the status to Accepted. That creates the job and carries the quote number, customer, address, hours and budget into the Job Cost Summary. Jane will not press that control for you." },
] as const;

export function Stage1WelcomeGuide({ onClose, onStepChange, mode = "dashboard", initialStep = 0, onJourneyAction }: { onClose: () => void; onStepChange: (step: number) => void; mode?: "dashboard" | "quotes"; initialStep?: number; onJourneyAction?: (step: number) => void }) {
  const { session } = useAuth();
  const slides = mode === "quotes" ? quotesSlides : dashboardSlides;
  ...
  return <aside ref={panelRef} style={positioned} className={`fixed z-[80] max-h-[calc(100vh-1rem)] w-[calc(100%-1.5rem)] max-w-2xl overflow-auto rounded-xl border border-sky-300 bg-white shadow-2xl ${position ? "" : "inset-x-3 bottom-3 mx-auto md:bottom-6"}`}>
    <div className="bg-[#061b34] px-5 py-4 text-white">
      <div onPointerDown={startDrag} onPointerMove={moveDrag} onPointerUp={endDrag} onPointerCancel={endDrag} className="mb-2 hidden cursor-move touch-none items-center justify-center gap-2 text-[10px] uppercase tracking-widest text-slate-300 md:flex"><GripHorizontal className="h-4 w-4" /> Drag this tour to a comfortable position</div>

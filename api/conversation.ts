...
- offered means Hudson may ask whether guidance would be useful but may not prescribe.
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (!first.contract) {
      ...
      return res.status(502).json({ error: "Hudson could not form a governed reply. Please try again." });
    }
    ...
    if (!policy.pass) {
      ...
      if (!second.contract) {
        return res.status(502).json({ error: "Hudson's reply was withheld because it did not meet the constitutional standard." });
      }
    ...
    if (!policy.pass) {
      ...
      return res.status(422).json({ error: "Hudson's reply was withheld because it did not meet the constitutional standard." });
    }
